...
## 2024-05-19 - Cache API requests securely
**Learning:** `window.apiCache` is initialized globally in `index.html` as `window.apiCache = window.apiCache || {};`. Do not assume it is undefined when adding new API caching logic.
**Action:** When implementing caching, safely access `window.apiCache` knowing it is already initialized.
